...
<Info>
Files are task-scoped by default. When you create a task and upload files, they're stored in that task's workspace and accessible only to that execution. Guest sessions created through share links or widgets follow the same task-scoped model for their uploaded files.
</Info>
...
- **Task-scoped by default** - Files are isolated per task or guest session
...
- [Sharing Agents](/agents/sharing) - Understand guest task behavior
- [Embed Widget](/agents/widget-embed) - Understand embedded guest sessions
