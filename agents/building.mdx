...
**Distribution Controls** (Optional)
- Configure whether the agent can be shared publicly
- Enable widget access for embedded use cases
- Restrict widget access to approved domains
...
After publishing, you can decide how the agent is consumed:
- Keep it internal for signed-in users
- Generate a share link for guest access
- Enable widget access for selected domains
- Create a runtime API key for SDK or server-side integration
...
The same published configuration is also reused when the agent is launched from a share link, from an embedded widget, or from a workforce run.
...
- A clear decision about internal-only, share-link, widget, or API use
...
- If using share links or widgets, test the guest entry flow separately
...
- [Sharing Agents](/agents/sharing) - Configure public share links
- [Embed Widget](/agents/widget-embed) - Enable embedded agent access
- [SDK Guide](/guides/sdk) - Call published agents through `/v1`
- [Workforces](/workforces/overview) - Reuse agents in multi-agent orchestration
